...
const TrainCar = memo(({ children, href }: TrainCarProps): React.ReactElement => {
  ...
  const asciiCar = `   ______________________________
  ||                            |
   |                            |
  ...
  const isBadgeConfigArray = (value: unknown): value is BadgeConfig[] => {
    return Array.isArray(value) && value.every(child => 'src' in child);
  };

  const validateBadges = useCallback((badges: BadgeConfig[] | React.ReactElement<ImgProps>) => {
  ...
  return (
    <ErrorBoundary>
      <div style={{ width: 240, position: 'relative', lineHeight: 1.2 }}>
        ...
        <pre className="font-mono m-0 text-gray-600">{asciiCar}</pre>
